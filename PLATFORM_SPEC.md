> **Repo focus — GDGWebsite (main site, Next.js 14):** owns **§5 Events UI**, **§6 Profile wiring**, **§9 Analytics dashboard UI**, **§10 Admin UI**. Consumes the auth service's new endpoints (§4); the profile placeholders in `lib/member.ts` light up once `/auth/me` returns `activity` + `certificates`.
>
