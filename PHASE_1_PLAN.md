> **Repo focus — GDGWebsite (main site, Next.js 14):** your workstreams are **F** (`lib/events-service.ts` + `/events` browse/detail server components with OG metadata), **G** (nav link + homepage upcoming-events section), **H** (minimal `/admin` — event CRUD, roster, check-in), and **I** (extend `PlatformUser` so the existing profile tiles light up). Do not start F until the auth repo's workstreams A–B are deployed/reachable; I depends on E.
>
